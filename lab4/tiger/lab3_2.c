...
int main(int argc, char *argv[]) {
    ...
    if(id == 0) {
        ...
        for(c = 1; c < p; c++) {
            for(i = c * row_part ; i < (c + 1) * row_part ; i++) {
                for(j = 0 ; j < COL ; j++) {
                    ...
                    MPI_Irecv(&arrC[i][j], 1, MPI_FLOAT, c, 5, MPI_COMM_WORLD, &request[0]);
        ...
        MPI_Irecv(&COL, 1, MPI_INT, 0, 1, MPI_COMM_WORLD, &request[1]);
		MPI_Irecv(&row_part, 1, MPI_INT, 0, 2, MPI_COMM_WORLD, &request[2]);
        ...
        for(i = 0 ; i < row_part ; i++) {
            for(j = 0 ; j < COL ; j++) {
                ...
                MPI_Irecv(&arrA[i][j], 1, MPI_FLOAT, 0, 3, MPI_COMM_WORLD, &request[3]);
				MPI_Irecv(&arrB[i][j], 1, MPI_FLOAT, 0, 4, MPI_COMM_WORLD, &request[4]);
